...
const totalDuration = (array) => {
  const adamModule = array.slice(1, 23);
  const przemekModule = array.slice(23, 47);
  const marcinModule = array.slice(47);

  const sumOfMin = (array) => {
    return array
      .map((lesson) => lesson.length)
      .map((hour) => {
  ...
  const numberToHour = (sum, moduleName) => {
    const hours = Math.floor(sum / 60);
    const minutes = sum % 60;
    return `${moduleName} ${hours} godzin ${minutes} minut`;
  };

  const lessonsDuration = numberToHour(
    sumOfMin(lessons),
    `Kurs Opanuj JavaScript trwa: `
  );
  const adamModuleDuration = numberToHour(
    sumOfMin(adamModule),
    `Moduł Adama: `
  );
  const przemekModuleDuration = numberToHour(
    sumOfMin(przemekModule),
    `Moduł Przemka: `
  );
  const marcinModuleDuration = numberToHour(
    sumOfMin(marcinModule),
    `Moduł Marcina: `
  );

  return console.log(`${lessonsDuration}
  ${adamModuleDuration}
  ${przemekModuleDuration}
  ${marcinModuleDuration}`);
};
